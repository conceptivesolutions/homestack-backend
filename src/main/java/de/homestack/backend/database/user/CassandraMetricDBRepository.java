...
class CassandraMetricDBRepository extends AbstractCassandraDBFacade implements IMetricDBRepository
{
  ...
  {
    return executeQuery(QueryBuilder.selectFrom(sessionProvider.getKeyspaceName(pUserID), _TABLE_METRICS_BY_DEVICEID)
  ...
  {
    executeUpdate(QueryBuilder.insertInto(sessionProvider.getKeyspaceName(pUserID), _TABLE_METRICS_BY_DEVICEID)
  ...
  {
    return executeUpdate(QueryBuilder.deleteFrom(sessionProvider.getKeyspaceName(pUserID), _TABLE_METRICS_BY_DEVICEID)
                             .whereColumn("deviceid").isEqualTo(QueryBuilder.literal(UUID.fromString(pDeviceID)))
                             .whereColumn("id").isEqualTo(QueryBuilder.literal(UUID.fromString(pMetricID)))
                             .build(), pUserID);
  }
